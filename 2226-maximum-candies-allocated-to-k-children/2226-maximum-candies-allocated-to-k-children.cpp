...
public:
    bool check(long long mid, vector<int>& c, long long k) {
        ...
        return false;  // Return false if we couldn't distribute enough
    }

    int maximumCandies(vector<int>& c, long long k) {
        long long sum = accumulate(c.begin(), c.end(), 0LL);
        if (sum < k) return 0;  // Not enough candies to give at least one to each child

        long long l = 1, r = *max_element(c.begin(), c.end()), ans = 0;

        while (l <= r) {
            long long mid = l + (r - l) / 2;
            if (check(mid, c, k)) {
                ans = mid;  // Store valid `mid`
                l = mid + 1;  // Try for a larger `mid`
            } else {
                r = mid - 1;  // Try a smaller `mid`
            }
        }
        return ans;
    }
